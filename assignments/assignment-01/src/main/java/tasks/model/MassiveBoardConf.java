package tasks.model;


import java.util.ArrayList;
import java.util.List;

public class MassiveBoardConf implements BoardConf {

	@Override
	public BallImpl getPlayerBall() {
		return  new BallImpl(new P2d(0, -0.75), 0.05, 1.5, new V2d(0,0));
	}

    @Override
    public BallImpl getBotBall() {
        return new BallImpl(new P2d(0, -0.75), 0.05, 1.5, new V2d(0,0.5));
    }

	@Override
	public List<BallImpl> getSmallBalls() {
		var ballRadius = 0.01;
        var balls = new ArrayList<BallImpl>();

    	for (int row = 0; row < 30; row++) {
    		for (int col = 0; col < 150; col++) {
        		var px = -1.0 + col*0.015;
        		var py =  row*0.015;
        		var b = new BallImpl(new P2d(px, py), ballRadius, 0.25, new V2d(0,0));
            	balls.add(b);    			
    		}
    	}		
    	return balls;
	}

    @Override
    public List<Hole> getHoles(){
        var holes = new ArrayList<Hole>();
        var h1 = new Hole(new P2d(-1.5, 1.0), 0.2);
        var h2 = new Hole(new P2d(1.5, 1.0), 0.2);
        holes.add(h1);
        holes.add(h2);
        return holes;
    }

    @Override
    public Boundary getBoardBoundary() {
        return new Boundary(-1.5,-1.0,1.5,1.0);
	}
}
